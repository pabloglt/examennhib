...
public class OpinionRepository {
    ...
    public List<Opinion> findByUsuarioEmail(String email) {
        try (Session session = sessionFactory.openSession()) {
            return session.createQuery("FROM Opinion o WHERE o.usuarioEmail = :email", Opinion.class)
    ...
    public List<Opinion> findOpinionesPuntuacionBajo(int score) {
        try (Session session = sessionFactory.openSession()) {
            return session.createQuery("FROM Opinion o WHERE o.puntuacion <= :score", Opinion.class)
                    .setParameter("puntuacion", score)
                    .list();
        }
    }
}